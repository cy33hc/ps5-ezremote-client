...
namespace Windows
{
    ...
    void ConnectionPanel()
    {
        ...
        if (ImGui::Button(id, ImVec2(width, 0)))
        {
            ...
            Dialog::initImeDialog(lang_strings[STR_SERVER], remote_settings->server, 255, SCE_IME_TYPE_URL, pos.x, pos.y);
        ...
        if (remote_settings->type != CLIENT_TYPE_NFS)
        {
            ...
            if (ImGui::Button(id, ImVec2(width, 0)))
            {
                ...
                ime_field_size = 32;
                ime_callback = SingleValueImeCallback;
                Dialog::initImeDialog(lang_strings[STR_USERNAME], remote_settings->username, 32, SCE_IME_TYPE_DEFAULT, pos.x, pos.y);
                gui_mode = GUI_MODE_IME;
            }
            ImGui::SameLine();
        }
        ...
        if (remote_settings->type != CLIENT_TYPE_NFS)
        {
            ...
            if (ImGui::Button(id, ImVec2(100, 0)))
            {
                ...
                ime_field_size = 127;
                ime_callback = SingleValueImeCallback;
                Dialog::initImeDialog(lang_strings[STR_PASSWORD], remote_settings->password, 127, SCE_IME_TYPE_DEFAULT, pos.x, pos.y);
                gui_mode = GUI_MODE_IME;
            }
        }
    ...
    void ShowActionsDialog()
    {
        ...
        if (ImGui::BeginPopupModal(lang_strings[STR_ACTIONS], NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            if (ImGui::Selectable(lang_strings[STR_EXTRACT], false, getSelectableFlag(REMOTE_ACTION_EXTRACT) | ImGuiSelectableFlags_DontClosePopups, ImVec2(220, 0)))
            {
                ...
                Dialog::initImeDialog(lang_strings[STR_EXTRACT_LOCATION], extract_zip_folder, 255, SCE_IME_TYPE_DEFAULT, 600, 350);
                gui_mode = GUI_MODE_IME;
            ...
            if (local_browser_selected)
            {
                ...
                if (ImGui::Selectable(lang_strings[STR_COMPRESS], false, getSelectableFlag(REMOTE_ACTION_NONE) | ImGuiSelectableFlags_DontClosePopups, ImVec2(220, 0)))
                {
                    ...
                    Dialog::initImeDialog(lang_strings[STR_ZIP_FILE_PATH], zip_file_path, 383, SCE_IME_TYPE_DEFAULT, 600, 350);
                    gui_mode = GUI_MODE_IME;
                    file_transfering = true;
                    SetModalMode(false);
                    ImGui::CloseCurrentPopup();
    ...
    void ShowFavoriteUrlsDialog()
    {
        if (select_url_inprogress)
        {
            ...
            if (ImGui::BeginPopupModal(lang_strings[STR_FAVORITE_URLS], NULL, ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                ImGui::Checkbox("##enable_alldebrid_install_url", &install_pkg_url.enable_alldebrid);
                ImGui::SameLine();
                ImGui::Text("%s", lang_strings[STR_ENABLE_ALLDEBRID_MSG]);

                ImGui::SameLine();
                ImGui::SetCursorPosX(ImGui::GetCursorPosX()+40);
                ImGui::Checkbox("##enable_realdebrid_install_url", &install_pkg_url.enable_realdebrid);
                ImGui::SameLine();
                ...
                if (ImGui::Button(lang_strings[STR_ONETIME_URL], ImVec2(1070, 0)))
                {
                    ...
                    ime_callback = SingleValueImeCallback;
                    Dialog::initImeDialog("URL", install_pkg_url.url, 511, SCE_IME_TYPE_DEFAULT, 600, 340);
                    gui_mode = GUI_MODE_IME;
            ...
            if (ImGui::BeginPopupModal(lang_strings[STR_EDITOR], NULL, ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                for (std::vector<std::string>::iterator it = edit_buffer.begin(); it != edit_buffer.end(); it++)
                {
                    ...
                    if (ImGui::Selectable(it->c_str(), false, ImGuiSelectableFlags_DontClosePopups, ImVec2(1275, 0)))
                    {
                        ...
                        ime_callback = SingleValueImeCallback;
                        Dialog::initImeDialog(lang_strings[STR_EDIT], edit_line, 1023, SCE_IME_TYPE_DEFAULT, 420, 290);
                        gui_mode = GUI_MODE_IME;
                    }
                    ImGui::PopStyleVar();
                    ImGui::PopID();
        ...
        if (show_settings)
        {
            ...
            if (ImGui::BeginPopupModal(lang_strings[STR_SETTINGS], NULL, ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                if (ImGui::Button(id, ImVec2(835-width, 0)))
                {
                    ...
                    ime_field_size = 512;
                    ime_callback = SingleValueImeCallback;
                    Dialog::initImeDialog(lang_strings[STR_COMPRESSED_FILE_PATH], temp_folder, 255, SCE_IME_TYPE_DEFAULT, 1050, 80);
                    gui_mode = GUI_MODE_IME;
                }
                ImGui::PopStyleVar();
                ImGui::Separator();
                ...
                if (ImGui::Button(id, ImVec2(835-width, 0)))
                {
                    ...
                    Dialog::initImeDialog(lang_strings[STR_BG_DOWNLOAD_MIN_SIZE], txt_bg_download_size, 16, SCE_IME_TYPE_NUMBER, 1050, 80);
                    gui_mode = GUI_MODE_IME;
                }
                ImGui::PopStyleVar();
                ImGui::Separator();
                ...
                if (ImGui::Button(id, ImVec2(835-width, 0)))
                {
                    ...
                    Dialog::initImeDialog(lang_strings[STR_PORT], txt_http_server_port, 5, SCE_IME_TYPE_NUMBER, 1050, 80);
                    gui_mode = GUI_MODE_IME;
                }
                ImGui::Separator();
                ...
                if (ImGui::Button(id, ImVec2(835-width, 0)))
                {
                    ...
                    ime_field_size = 512;
                    ime_callback = SingleValueImeCallback;
                    Dialog::initImeDialog(lang_strings[STR_COMPRESSED_FILE_PATH], compressed_file_path, 512, SCE_IME_TYPE_DEFAULT, 1050, 80);
                    gui_mode = GUI_MODE_IME;
                }
                ImGui::PopStyleVar();
                ImGui::Separator();
                ...
                if (ImGui::Button(id, ImVec2(835-width, 0)))
                {
                    ...
                    ime_field_size = 63;
                    ime_callback = SingleValueImeCallback;
                    Dialog::initImeDialog(lang_strings[STR_API_KEY], alldebrid_api_key, 63, SCE_IME_TYPE_DEFAULT, 1050, 80);
                    gui_mode = GUI_MODE_IME;
                }
                ImGui::PopStyleVar();
                ImGui::Separator();
                ...
                if (ImGui::Button(id, ImVec2(835-width, 0)))
                {
                    ...
                    ime_field_size = 63;
                    ime_callback = SingleValueImeCallback;
                    Dialog::initImeDialog(lang_strings[STR_API_KEY], realdebrid_api_key, 63, SCE_IME_TYPE_DEFAULT, 1050, 80);
                    gui_mode = GUI_MODE_IME;
                }
                ImGui::PopStyleVar();
                ImGui::Separator();
    ...
    void ExecuteActions()
    {
        ...
        case ACTION_NEW_REMOTE_FILE:
            if (gui_mode != GUI_MODE_IME)
            {
                ...
                Dialog::initImeDialog((selected_action == ACTION_NEW_LOCAL_FILE || selected_action == ACTION_NEW_REMOTE_FILE)? lang_strings[STR_NEW_FILE]: lang_strings[STR_NEW_FOLDER], dialog_editor_text, 128, SCE_IME_TYPE_DEFAULT, pos.x, pos.y);
                gui_mode = GUI_MODE_IME;
            }
            break;
        ...
        case ACTION_RENAME_LOCAL:
            if (gui_mode != GUI_MODE_IME)
            {
                ...
                ime_cancelled = CancelActionCallBack;
                ime_callback = SingleValueImeCallback;
                Dialog::initImeDialog(lang_strings[STR_RENAME], dialog_editor_text, 128, SCE_IME_TYPE_DEFAULT, 410, 350);
                gui_mode = GUI_MODE_IME;
            }
            break;
        case ACTION_RENAME_REMOTE:
            if (gui_mode != GUI_MODE_IME)
            {
                ...
                ime_cancelled = CancelActionCallBack;
                ime_callback = SingleValueImeCallback;
                Dialog::initImeDialog(lang_strings[STR_RENAME], dialog_editor_text, 128, SCE_IME_TYPE_DEFAULT, 1330, 350);
                gui_mode = GUI_MODE_IME;
            }
            break;
